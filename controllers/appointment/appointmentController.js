...
// Appointment listing (admin / doctor side)
...
// Patient's appointments by auth / createdBy
export const getAppointmentById = async (req, res) => {
  try {
    const queryCreateBy = req.query.createdBy || null;
    const clerkUserId = req.auth?.userId || null;
    const resolvedCreatedBy = queryCreateBy || clerkUserId || null;
    ...
    console.error("Get Appointment by patient Error:", err);
    res.status(500).json({
      success: false,
      error: "server error",
    });
...
// Create an appointment (Cash or Stripe Online)
export const createAppointment = async (req, res) => {
  try {
    ...
    const numbericFee = safeNumber(fee ?? fees ?? 0);
    if (numbericFee === null || numbericFee < 0) {
      return res.status(400).json({
        success: false,
    ...
    if (existingBooking) {
      return res.status(400).json({
        success: false,
        error: "you already have appointment with this doctor at the selected slot.",
      });
    }
...
// Confirm online payment from Stripe
export const confirmPayment = async (req, res) => {
  try {
    ...
    // Last attempt: find appointment created recently with matching amount
    if (!appt) {
...
// Update appointment (status/notes/date/time)
...
// Register user count (from Clerk)
