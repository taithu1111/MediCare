...
//HELPER FUNCTIONS
//this func will return a finite number 
...
// this func will parse the time string and return AM : PM
...
// this func will build the frontend base url
const buildFrontendBase = (req) => {
    const env = process.env.FRONTEND_URL;
    if (env) return env.replace(/\/$/, "");
    const origin = req.get("origin") || req.get("referrer") || null;
    return origin ? origin.replace(/\/$/, "") : null;
};

//this func will get the user from clerk and return the user details 
...
export const confirmServicePayment = async (req, res) => {
    try {
        ...
        if (session.payment_status != "paid") return res.status(402).json({
            success: false,
...
export const getServiceAppointments = async (req, res) => {
    try {
        ...
        console.error("Get service appointment err:", err);
...
export const getServiceAppointmentById = async (req, res) => {
    try {
        ...
        console.error("Get service appointment by id err:", err);
        return res.status(500).json({
            success: false,
            message: "Server error"
        })
...
export const updateServiceAppointment = async (req, res) => {
    try {
        ...
        console.error("Update service appointment err:", err);
        return res.status(500).json({
            success: false,
            message: "Server error"
        })
...
//to cancle the service appointment
export const cancelServiceAppointment = async (req, res) => {
    try {
        ...
        console.error("Cancel service appointment err:", err);
        return res.status(500).json({
            success: false,
            message: "Server error"
        })
...
export const getServiceAppointmentStats = async (req, res) => {
    try {
        const services = await Service.aggregate([
            ...
            { $sort: { createdAt: -1 } },
        ]);//it will give you total appointment, completed appt
        // cancle appt and total earning
        return res.json({ success: true, services });
    } catch (err) {
        console.error("Get service appointment stats err:", err);
        return res.status(500).json({
            success: false,
            message: "Server error"
        })
...
export const getServiceAppointmentByPatient = async (req, res) => {
    try {
        ...
        console.error("Get ServiceAppointment Stats Error:", err);
        return res.status(500).json({
            success: false,
            message: "Server error"
        })
